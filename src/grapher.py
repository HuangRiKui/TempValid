...
class Grapher(object):
    def __init__(self, dataset_dir):
        ...
        self.id2entity = dict([(int(v), k) for k, v in self.entity2id.items()])
        self.id2relation = dict([(int(v), k) for k, v in self.relation2id.items()])
    ...
    def create_store(self, file):
        ...
        # store_idx = store_idx[np.argsort(store_idx[:, -1])]
