...
class Rule_Learner(object):
    ...
    def estimate_confidence(self, rule, num_samples=500):
        ...
        beta = 0.1
        if body_support:
            rule_support, latest_tss_list= self.calculate_rule_support(unique_bodies, rule["head_rel"])
            confidence = round(rule_support / body_support, 6)
            latest_tss = dict(Counter(latest_tss_list))
            if len(latest_tss)>2:
                tss_key = sorted(list(latest_tss.keys()))
                tss_1 = tss_key[0]
                tss_2 = tss_key[1]
                beta = (np.log(latest_tss[tss_2])-np.log(latest_tss[tss_1]))/(tss_1-tss_2)
                if beta<0:
                    tss_1 = tss_2
                    tss_2 = tss_key[2]
                    beta = (np.log(latest_tss[tss_2]) - np.log(latest_tss[tss_1])) / (tss_1 - tss_2)
        ...
        return confidence, rule_support, body_support, beta
    ...
    def calculate_rule_support(self, unique_bodies, head_rel):
        ...
        for body in unique_bodies:
            ...
            if True in mask:
                rule_support += 1
                latest_tss = np.min(head_rel_edges[mask][:,3])
                latest_tss_list.append(latest_tss-body[1])

        return rule_support, latest_tss_list
    ...
    def save_rules_verbalized(self):
        ...
        #     dt, rule_lengths, num_walks, transition_distr, seed
        # )
        filename = "rules_visualization.txt"
        filename = filename.replace(" ", "")
        with open(self.output_dir + filename, "w", encoding="utf-8") as fout:
...
def rules_statistics(rules_dict):
    ...
    print("Total number of rules: ", sum([rule_length[1] for rule_length in rule_lengths]))
